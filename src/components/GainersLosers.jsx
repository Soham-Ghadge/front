    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between mb-8 mx-4">
        <div className="w-1/2 border border-gray-300 rounded-lg p-6">
          <h2 className="text-4xl font-semibold mb-4 text-green-500">Top Gainers</h2>
          <ul>
            {gainers.map((ticker, index) => (
              <li key={index} className="text-gray-800 flex justify-between">
                ...
                <span className="text-green-500">{ticker.percent_change_24h}%</span>
              </li>
            ))}
          </ul>
        </div>
        <div className="w-1/2 border border-gray-300 rounded-lg p-6">
          <h2 className="text-4xl font-semibold  mb-4 text-red-500">Top Losers</h2>
          <ul>
            {losers.map((ticker, index) => (
              <li key={index} className="text-gray-800 flex justify-between">
                <span className="text-white">{ticker.name}</span>
                <span className="text-red-500">{ticker.percent_change_24h}%</span>
              </li>
            ))}
          </ul>
        </div>
